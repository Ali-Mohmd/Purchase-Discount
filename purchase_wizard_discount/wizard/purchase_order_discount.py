...
class PurchaseOrderDiscount(models.TransientModel):
    ...
    def _prepare_discount_line_values(self, product, amount, taxes, description=None):
        ...
        if description:
            # If not given, name will fallback on the standard POL logic (cf. _compute_name)
    ...
    def _get_discount_product(self):
        ...
        if not discount_product:
            if (
                ...
                and self.company_id.check_field_access_rights('write', ['purchase_discount_product_id'])
            ):
    ...
    def _create_discount_lines(self):
        ...
        if self.discount_type == 'amount':
            ...
            for line in self.purchase_order_id.order_line.filtered(lambda l: not l.avoid_global_discount):
            ...
            context = {'lang': self.purchase_order_id._get_lang()}  # noqa: F841
    ...
    def action_apply_discount(self):
        ...
        if self.discount_type == 'pol_discount':
            applicable_lines = self.purchase_order_id.order_line.filtered(lambda l: not l.avoid_global_discount)
            applicable_lines.write({'discount': self.discount_percentage*100})
        else:
